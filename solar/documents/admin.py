...
class ProjectDocumentInline(admin.StackedInline):
    ...
    extra = 0
    # Removido 'file_name' e 'file_size' de fields, pois serão readonly_fields via métodos
    fields = (
        'document_type', 'file', 'file_type',
        'description', 'is_approved', 'rejection_reason'
    )
    # Atualizado para usar os métodos 'file_display' e 'file_size_display'
    readonly_fields = ('file_type', 'file_display', 'file_size_display', 'uploaded_at')
    ...
    # Adicionado métodos file_display e file_size_display para o inline
    def file_display(self, obj):
        """Exibe nome do arquivo com link para o inline"""
        if obj.file:
    ...
    def file_size_display(self, obj):
        """Exibe tamanho do arquivo formatado para o inline"""
        if obj.file:
            size = obj.file.size
            if size < 1024:
                return f'{size} B'
...
class ClientProjectAdmin(admin.ModelAdmin):
    ...
    def documento_validation_status(self, obj):
        """Mostra status de validação do documento"""
        if obj.documento:
            is_valid = obj.is_documento_valid()
            if is_valid:
                return format_html(
    ...
    def documents_count(self, obj):
        ...
        required = len(obj.get_required_documents())
        if total > 0:
            return format_html(
    ...
    def convert_coordinates(self, request, queryset):
        """Converte coordenadas entre decimal e graus/min/seg"""
        updated = 0
        for project in queryset:
            try:
...
class ProjectDocumentAdmin(admin.ModelAdmin):
    ...
    def file_display(self, obj):
        """Exibe nome do arquivo com link"""
        if obj.file:
            file_name = obj.file.name.split('/')[-1]
            return format_html(
                '<a href="{}" target="_blank">{}</a>',
    ...
    def download_documents(self, request, queryset):
        """Action para download de documentos"""
